...
const BlogTitles = () => {
const blogCatagories=['General','Technology','Business','Health','Lifestyle','Education','Travel','Food']
...
const onSubmitHandler=async(e)=>{
  ...
  try {
    setLoading(true)
    const prompt=`Generate a blog title for the keyword ${input} in the catagory ${selectedCategory}`
  ...
  return (
     <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form  onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-6 text-sm font-medium'>Catagory</p>
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {
            blogCatagories.map((item)=>(
              <span onClick={()=>setSelectedCategory(item)} 
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory===item?'bg-blue-50 text-purple-700':'text-gray-500 border-gray-300'}`} 
              key={item}>{item}</span>
            ))
          }
        </div>
