...
const GenerateImages = () => {
  ...
  const imageStyle=['Realistic','Ghibli style','Anime style','Cartoon style','Fantasy style','Realistic Style','3D Style','Portrait style']
    ...
    const [pushlish,setPublish]=useState(false);
  ...
  const onSubmitHandler=async(e)=>{
    ...
    try {
      ...
      const {data}=await axios.post('/api/ai/generate-image',{prompt,pushlish},
      ...
      <form  onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            <input type="checkbox" onChange={(e)=>setPublish(e.target.checked)} checked={pushlish} className='sr-only peer'/>
